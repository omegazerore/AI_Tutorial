...
self_ids_check = [False] * N

group_number = 0

starting_point = 0

while sum(self_ids_check) < N:
    id = starting_point
    self_ids_check[starting_point] = True
    while True:
        if friendship[id] != starting_point:
            id = friendship[id]
            self_ids_check[id] = True
        else:
            group_number += 1
            for idx, bool_ in enumerate(self_ids_check):
                if not bool_:
                    starting_point = idx
                    break
            break
