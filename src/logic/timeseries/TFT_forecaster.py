...
class TFTForecaster(BaseForecaster):
    ...
    MODEL_FOLDER = os.path.join(get_datafetch(), 'model', 'TFT')
    NAME = 'TFT'
    ...
    def _pipeline(self):
        ...
        # Build TFT model
        self._build_model()
        logger.info("TFT model is built.")
    ...
    def _load_best_model(self, filename: str):
        ...
        """
        self.best_model = TemporalFusionTransformer.load_from_checkpoint(filename)
    ...
    def _build_model(self):
        ...
        net = TemporalFusionTransformer.from_dataset(
            ...
            hidden_size=self._hidden_size,  # most important hyperparameter apart from learning rate
            attention_head_size=self._attention_head_size,
            dropout=self._dropout,  # between 0.1 and 0.3 are good values
            hidden_continuous_size=self._hidden_size,  # set to <= hidden_size
    ...
    def optimize_hyperparameters(self, df: pd.DataFrame, n_trials: int = 20):
        ...
        self._df = df
        self._load_training_config()
        train_dataloader, val_dataloader = self._build_train_val_dataloader(self._training, self._df, predict=False)
...
if __name__ == "__main__":
    ...
    tft.optimize_hyperparameters(df=df_train, n_trials=5)
